...
HELM_CHART_NAME ?= opentelemetry-demo
...
generate-kubernetes-manifests: helm-update
	...
	$(HELM_CMD) template opentelemetry-demo $(HELM_REPO_NAME)/$(HELM_CHART_NAME) --namespace $(KUBERNETES_NAMESPACE) | \
		sed '/helm.sh\/chart\:/d' | sed '/helm.sh\/hook/d' | \
		sed '/managed-by\: Helm/d' >> kubernetes/opentelemetry-demo.yaml
...
.PHONY: kubernetes-undeploy
kubernetes-undeploy:
	$(HELM_CMD) uninstall opentelemetry-demo --namespace $(KUBERNETES_NAMESPACE)
